    ...
    def __init__(self, width, height, *args):
        ...
        for rover_detail in self.rover_details:
            try:  # Checking if some of the rover details are missing, if Yes, Exception will be thrown
                ...
                # check if the provided direction is not correct -> if Yes -> proceed with the other directions and commands.
                ...
                for action in self.rover_moves:
                    ...
                    # check if the provided move is not correct -> if Yes -> proceed with the other actions
                    if action not in ["L", "R", "M"]:
                        print("The action provided is not correct!")
                        continue
                    if action == 'L':
...
mars_rover6 = MarsRover(5, 5, [6, 4, 'N', 'LMLMLMLMM'])
mars_rover7 = MarsRover(5, 5, [5, 4, 'B', 'LMLMLMLMM'])
mars_rover8 = MarsRover(5, 5, [5, 4, 'S', 'AMLMLMLMM'])